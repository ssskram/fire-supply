...
export const actionCreators = {
  isUserAdmin: (user): AppThunkAction<any> => async dispatch => {
    const response = await fetch(
      "https://365proxy.azurewebsites.us/pghsupply/isAdmin?user=" + user.email,
      {
        method: "get",
        headers: new Headers({
    ...
    dispatch({
      type: constants.setAdminStatus,
      adminStatus: adminStatus.isAdmin
    });
  },
  loadUserProfile: (user): AppThunkAction<any> => async dispatch => {
    const response = await fetch(
      "https://mongo-proxy.azurewebsites.us/get/userProfile?user=" + user.email,
      {
        method: "get",
        headers: new Headers({
  ...
  updateCart: (newProfile): AppThunkAction<any> => async (
    dispatch,
    getState
  ) => {
...
export const reducer: Reducer<types.userProfile> = (
  state: any,
