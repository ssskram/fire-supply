...
export default class OrderCard extends React.Component<props, state> {
  ...
  render() {
    ...
    let lastSix;
    if (order._id) {
      lastSix = order._id.substr(order._id.length - 6);
    } else {
      lastSix = "...loading";
    }
    return (
      <div className="col-md-12 text-center">
        <div
          ...
          style={{ borderColor: colorByStatus(order.status) }}
        ...
        >
          <div className="panel-body" style={style.panelBody}>
            <div className="col-md-3" style={style.paddingTen}>
              {order.emergencyOrder && (
                <div style={style.Emergency}>
                  <b className="emergency oswald">EMERGENCY</b>
                </div>
              )}
            ...
            <div
              className={
                innerWidth < 990
                  ? "col-md-4 col-md-offset-2 funny-panel-small"
                  : "col-md-4 col-md-offset-2 funny-panel-big"
              }
              style={{ backgroundColor: colorByStatus(order.status) }}
            >
              <div style={{ alignSelf: "center" }}>
                <div>Order #{lastSix}</div>
                <div style={style.smallerFont}>Status</div>
                <div style={style.biggerFont}>
                  <b style={{ textTransform: "uppercase" }}>{order.status}</b>
                </div>
              </div>
            </div>
          </div>
