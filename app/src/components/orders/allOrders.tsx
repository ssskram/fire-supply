...
export class AllOrders extends React.Component<props, state> {
  constructor(props) {
    ...
    this.state = {
      currentPage: 1,
      ordersByDept: props.orders.filter(
  ...
  componentWillReceiveProps(nextProps) {
    if (this.state.onFilter == false) {
      this.setState({
        ordersByDept: nextProps.orders.filter(
  ...
  render() {
    ...
    return (
      <div className="col-md-12">
        <Header text="ALL ORDERS" userProfile={this.props.userProfile} />
        <hr />
        <HydrateStore />
        <Filters
        ...
        {ordersByDept.length == 0 && <NoOrders />}
